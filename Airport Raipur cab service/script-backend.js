...
(function(){
  ...
  ready(function(){
    ...
    if(form){ form.addEventListener('submit', function(e){ var loc=(document.getElementById('location')?.value||'').trim(); var isLL=/^-?\d+(?:\.\d+)?,\s*-?\d+(?:\.\d+)?$/.test(loc); var isUrl=/^https?:\/\//i.test(loc) && /google\.com\/maps|maps\.app\.goo\.gl/i.test(loc); var isPlain=loc.length>=3; if(!isLL && !isUrl && !isPlain){ e.preventDefault(); setNotice('Enter an address, Google Maps link, or coordinates', false); } }); }
  });
})();